...
async def process_sample(sample: dict, coco_dir: str, base_client: AsyncOpenAI, 
                        trained_client: AsyncOpenAI, model_name: str):
    """并行处理单个样本，同时请求两个模型"""
    ...
    # 处理图片路径（POPE的image_id是12位字符串，补零）
    image_filename = f"{image_id:012d}.jpg"  # 关键：POPE的image_id需要补零到12位
    ...
    base_resp, trained_resp = await asyncio.gather(
        get_model_response(base_client, model_name, content),
        get_model_response(trained_client, model_name, content)
    )
...
async def main():
    ...
    parser.add_argument("--coco-dir", type=str, required=True, 
                        help="Path to COCO val2014 images directory (e.g., ./coco/val2014)")
    ...
    parser.add_argument("--model-name", type=str, default="llava-1.5-7b")
    ...
    if args.limit:
        dataset = dataset.select(range(args.limit))  # 测试时限制样本数
    ...
    for sample in dataset:
        task = process_sample(sample, args.coco_dir, base_client, trained_client, args.model_name)
        tasks.append(task)
