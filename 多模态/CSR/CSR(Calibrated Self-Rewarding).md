...
#### 📊实验
##### 实验设置
- **骨干模型**：核心采用 LLaVA-1.5 7B/13B，同时在 Vila 7B 上验证跨模型兼容性；
- **微调方式**：LoRA 低秩适配微调，单张 A100 80GB GPU 即可完成训练，7B 模型 3 轮迭代仅需 3.5 小时，13B 模型约 5 小时；
- **训练数据**：从 LLaVA150k 数据集的详细描述、复杂推理子类中采样约 13000 个图文样本，每一轮迭代使用完全相同的 prompt 与图像；
- **评估基准**：覆盖 3 大类共 10 个主流基准。
