...
| 核心配置                  | 关键操作                                                                     |
| --------------------- | ------------------------------------------------------------------------ |
| 224×224×3 RGB 图像      | 原图先缩放到短边 256，裁剪中心 224×224 区域，做像素均值中心化                                    |
| 96 个 11×11×3 卷积核，步长 4 | ReLU 激活 → 局部响应归一化（LRN） → 3×3 步长 2 的重叠最大池化，双 GPU 各分配 48 个卷积核              |
| 256 个 5×5×48 卷积核      | 仅与同 GPU 的 Conv1 输出连接，ReLU 激活 → LRN → 3×3 步长 2 的重叠最大池化，双 GPU 各分配 128 个卷积核 |
| 384 个 3×3×256 卷积核     | 与双 GPU 的 Conv2 输出全连接，ReLU 激活，双 GPU 各分配 192 个卷积核                          |
| 384 个 3×3×192 卷积核     | 仅与同 GPU 的 Conv3 输出连接，ReLU 激活，双 GPU 各分配 192 个卷积核                          |
| 256 个 3×3×192 卷积核     | 仅与同 GPU 的 Conv4 输出连接，ReLU 激活 → 3×3 步长 2 的重叠最大池化，双 GPU 各分配 128 个卷积核       |
| 4096 个神经元             | 与 Conv5 输出全连接，ReLU 激活，加入 Dropout 正则                                      |
| 4096 个神经元             | 与 FC6 全连接，ReLU 激活，加入 Dropout 正则                                          |
| 1000 个神经元             | 与 FC7 全连接，接入 1000 路 softmax，输出 1000 个类别的概率分布                             |
